...
::
    ...
    python scripts/util/lora_soup.py \\
        ...
        [--block-scale '*attn1*=0.0'] [--scale-file scales.json]
...
def parse_scale_file(path: Path) -> tuple[list[tuple[str, float]], list[tuple[int, str, float]]]:
    """Read block scales from a JSON file instead of the command line.

    ``--block-scale`` and ``--input-block-scale`` are the right shape for a
    person naming a group or two by hand. They are the wrong shape for a
    program, because a fitted group's patterns are its **member prefixes** --
    one per layer, deliberately, since a glob inferred from a naming rule can
    select the wrong layers while looking correct (``block_groups``). So a
    coefficient on one coarse group of a 224-layer adapter is ~36 arguments,
    and a per-input grid multiplies that by the number of inputs: three
    adapters over six groups is ~650 arguments, about 52 KB of command line.
    Windows' ``CreateProcess`` refuses past 32,767 characters, and the box is
    Windows -- so the natural use of the grid this feeds would fail at spawn,
    with an error about the command line rather than about the merge.

    The file has no such ceiling and needs no grammar: ``PATTERN=COEFF`` splits
    on ``=`` and ``INDEX:PATTERN=COEFF`` on ``:``, so those two characters
    cannot appear in a pattern passed as an argument. Here they can.

    Shape, with both keys optional::

        {"block_scales":       [{"pattern": "...", "coefficient": 0.5}],
         "input_block_scales": [{"save_index": 0, "pattern": "...", "coefficient": 0.5}]}

    Refusals are per-entry and name the index, because a table generated by a
    caller is read by whoever wrote the generator, not by whoever typed it.
    """
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
    except OSError as e:
        raise SoupError(f"--scale-file {path}: {e}") from e
    except json.JSONDecodeError as e:
        raise SoupError(f"--scale-file {path} is not valid JSON: {e}") from e
    if not isinstance(raw, dict):
        raise SoupError(f"--scale-file {path} must hold a JSON object, got {type(raw).__name__}")

    unknown = set(raw) - {"block_scales", "input_block_scales"}
    if unknown:
        # Not ignored: a misspelled key is a table of coefficients that silently
        # did nothing, and the merge would report success having applied none of
        # what the caller asked for.
        raise SoupError(
            f"--scale-file {path}: unknown key(s) {sorted(unknown)}; expected "
            "'block_scales' and/or 'input_block_scales'"
        )

    def _entry(where: str, i: int, item: object, *, indexed: bool) -> tuple:
        if not isinstance(item, dict):
            raise SoupError(f"--scale-file {path}: {where}[{i}] must be an object")
        pattern = item.get("pattern")
        coefficient = item.get("coefficient")
        if not isinstance(pattern, str) or not pattern:
            raise SoupError(f"--scale-file {path}: {where}[{i}] has no 'pattern'")
        # bool is an int in Python and would read as index 0 / coefficient 1.
        if isinstance(coefficient, bool) or not isinstance(coefficient, (int, float)):
            raise SoupError(
                f"--scale-file {path}: {where}[{i}] 'coefficient' is not a number"
            )
        if not indexed:
            return (pattern, float(coefficient))
        save_index = item.get("save_index")
        if isinstance(save_index, bool) or not isinstance(save_index, int):
            raise SoupError(
                f"--scale-file {path}: {where}[{i}] 'save_index' is not an integer"
            )
        return (save_index, pattern, float(coefficient))

    block_scales = [
        _entry("block_scales", i, item, indexed=False)
        for i, item in enumerate(raw.get("block_scales") or [])
    ]
    input_block_scales = [
        _entry("input_block_scales", i, item, indexed=True)
        for i, item in enumerate(raw.get("input_block_scales") or [])
    ]
    return block_scales, input_block_scales
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="read block scales from a JSON file: {'block_scales': [{'pattern','coefficient'}], "
             "'input_block_scales': [{'save_index','pattern','coefficient'}]}. For callers -- a "
             "fitted group is one pattern per layer, which exceeds the command-line limit as a "
             "grid. Composes with the flags above",
    )
    ...
    try:
        ...
        indexed_scales = [parse_input_block_scale(spec) for spec in args.input_block_scale]
        if args.scale_file:
            # Appended, not merged: scales compose by *multiplication* and the
            # product is order-independent, so a file entry and a flag entry on
            # the same pattern both apply rather than one winning.
            from_file, indexed_from_file = parse_scale_file(Path(args.scale_file))
            block_scales += from_file
            indexed_scales += indexed_from_file

        per_input: list[list[tuple[str, float]]] = [[] for _ in specs]
        for index, pattern, coefficient in indexed_scales:
