...
The question this answers (cinema-studio 469 follow-on): a LoKr delta is
...
def kron_spectrum(
    ...
    dropped = {
        ...
        for (p, _c), l in zip(adapters, loaded)
    ...
    for prefix in sorted(shared_set):
        if all(isinstance(l.layers[prefix], lora_soup.LokrLayer) for l in loaded):
    ...
    for prefix in lokr_shared:
        contributors = [l.layers[prefix] for l in loaded]
        w1_shape: tuple[int, ...] | None = None
        w2_shape: tuple[int, ...] | None = None
        w1s: list[Tensor] = []
        w2s: list[Tensor] = []
        cs: list[float] = []
        for loaded_i, layer in zip(loaded, contributors):
