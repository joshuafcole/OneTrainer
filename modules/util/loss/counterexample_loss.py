...
* **No pairing, no KL baseline, one knob.** ``beta``. The concept's own
  ``loss_weight`` survives as a multiplier on top, unchanged.
...
range. The default, 1000.0, is borrowed from Diffusion-DPO's ``beta_T`` for the
same element-mean convention; it is a starting convention, not a measured
optimum, and a single measurement on an SD 1.5 LoRA wanted roughly 30x more.
...
actually running at -- and a first short run reads it straight off.
``~1.0`` means beta is too small to ever switch off.
...
def counterexample_stats(delta: Tensor, losses: Tensor, beta: float) -> CounterexampleStats:
    """Summarize one batch's counterexample rows.

    ``delta`` and ``losses`` are already restricted to those rows by the caller.
    """
    if delta.numel() == 0:
        return EMPTY_STATS
