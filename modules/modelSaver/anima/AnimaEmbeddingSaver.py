...
class AnimaEmbeddingSaver(
    ...
    def _to_state_dict(
        ...
        # "qwen" -- the Qwen3 word-embedding table is the only table Anima trains into, and the same key
        # AnimaLoRASaver bundles under (bundle_emb.<placeholder>.qwen) and scripts/extract_bundled_embeddings.py
        # unpacks back to.
        if embedding is not None and embedding.text_encoder_embedding.vector is not None:
