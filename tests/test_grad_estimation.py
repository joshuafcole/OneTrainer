...
regimes that matter for the GA (gradient-aligned) init pass: plain forward,
gradient checkpointing (both reentrant modes), and a PeftBase-style replaced
forward with an additive zero adapter branch. Pure torch, CPU-only. Run with
``python -m pytest tests/test_grad_estimation.py``.
"""
...
from modules.util.grad_estimation import WeightGradientEstimator  # noqa: E402
...
def test_matches_autograd_with_compiled_module_under_force_eager():
    ...
    # ("Compilation of intermediate hooks requires compiled autograd"), so
    # GenericTrainer's estimation pass runs under set_stance("force_eager").
    # Verify the hooks both fire and produce correct gradients in that stance.
    model = _make_model()
    expected = _reference_grads(model, steps=2)
